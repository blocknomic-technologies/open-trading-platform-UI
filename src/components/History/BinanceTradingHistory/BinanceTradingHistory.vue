...
export default {
  ...
  methods: {
    ...
    async changeSelectedPair(pair){
      ...
      let data = [];
      data = await HistoryService.binanceTradingHistoryData('binance', newVal);
      let parseData = [];
      if(data.data.error) {
    ...
    updateData() {
      this.history = this.initialData.filter((ele) => {
        return ele.orderId.toString().includes(this.searchString) || ele.commissionAsset.toLowerCase().includes(this.searchString.toLowerCase());
      });
