...
export default {
  ...
  async created() {
    ...
    this.openPositions = (await TradeService.getOpenPositions(
      this.selectedMarginExchange.value
    )).data;
  ...
  methods: {
    async getActiveOrders() {
      ...
      this.activeOrders = (await TradeService.getActiveOrders(
    ...
    async getActiveOrdersByNotify(data) {
      ...
      if (
        data.data.updateType === 'pnl' &&
        data.data.exchange.toLowerCase() ===
          this.selectedMarginExchange.value.toLowerCase()
      ) {
        this.isUpdating = true;
        this.openPositions = (await TradeService.getOpenPositions(
          this.selectedMarginExchange.value
    ...
    handleKeyDown(event) {
      if (
        (event.key === 'o' && event.altKey) ||
        (event.altKey && event.which == 79)
      ) {
